...
func (n *Livelint) checkCanSeeBackends(ingressName, namespace string) CheckResult {
	ingress, err := n.k8s.NetworkingV1().Ingresses(namespace).Get(context.Background(), ingressName, v1.GetOptions{})
	if err != nil {
	...
	for _, rules := range ingress.Spec.Rules {
		...
		for _, path := range rules.HTTP.Paths {
			if path.Backend.Service != nil {
				service, err := n.k8s.CoreV1().Services(namespace).Get(context.Background(), path.Backend.Service.Name, v1.GetOptions{})
				if err != nil {
					log.Fatal(fmt.Errorf("error getting backends for ingress %s in namespace %s: %w", ingressName, namespace, err))
				}
				...
				endpoint, err := n.k8s.CoreV1().Endpoints(namespace).Get(context.Background(), path.Backend.Service.Name, v1.GetOptions{})
