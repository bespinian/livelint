...
func (n *Livelint) getServicesForSelector(namespace string, matchLabels map[string]string) ([]apiv1.Service, []apiv1.Service, error) {
	...
	for _, service := range allServices.Items {
		...
		deploymentLabelsSubsetServiceLabels = isSubmap(serviceMatchLabels, matchLabels)
		serviceLabelsSubsetDeploymentLabels = isSubmap(matchLabels, serviceMatchLabels)
